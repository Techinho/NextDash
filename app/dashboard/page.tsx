...
export default function DashboardPage() {
  ...
  return data.isAdmin ? (
    ...
    <UserDashboard data={data} router={router} />
  )
}

/* ---------------- Normal user dashboard ---------------- */

function UserDashboard({ data, router }: { data: DashboardData; router: ReturnType<typeof useRouter> }) {
  const dailyLimit = 50
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Overview of your database access and usage.</p>
          </div>

          {/* Daily Limit Badge */}
          <div
            className={`w-fit flex items-center gap-3 px-4 py-2 rounded-full border ${
              isNearLimit ? "bg-red-50 border-red-200 text-red-700" : "bg-surface border-border text-foreground"
            } shadow-sm`}
          >
            <span className="text-xs font-semibold uppercase tracking-wider">Daily Credits</span>
            <span className="text-sm font-bold">
              {data.contactsViewedToday} / {dailyLimit}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          {/* Recent Contacts (blurred, unlock -> upgrade modal) */}
              ...
              <ul className="divide-y divide-border">
                {data.recentContacts.length === 0 ? (
                  ...
                  data.recentContacts.map((contact) => (
                    <li
                      key={contact.id}
                      className="px-4 sm:px-6 py-4 hover:bg-gray-50/50 transition-colors flex items-center justify-between group"
                    >
              ...
              <ul className="divide-y divide-border">
                {data.recentAgencies.length === 0 ? (
                  ...
                  data.recentAgencies.map((agency) => (
                    <li
                      key={agency.id}
                      className="px-4 sm:px-6 py-4 hover:bg-gray-50/50 transition-colors flex items-center justify-between"
                    >
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Compact overview of directory size and latest changes. No usage limits apply to admins.
            </p>
          </div>
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-border bg-surface text-xs text-muted-foreground">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="bg-surface border border-border rounded-lg p-4 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-semibold text-foreground">{formatMetric(data.contacts)}</p>
          </div>
          <div className="bg-surface border border-border rounded-lg p-4 shadow-sm">
            <p className="text-xs font-medium text-muted-foreground uppercase">Tracked Agencies</p>
            <p className="mt-2 text-2xl font-semibold text-foreground">{formatMetric(data.agencies)}</p>
          </div>
          <div className="bg-surface border border-border rounded-lg p-4 shadow-sm">
            <p className="text-xs font-medium text-muted-foreground uppercase">End‑User Limit</p>
            <p className="mt-2 text-sm text-muted-foreground">
              Standard users see up to <span className="font-semibold">50 contacts/day</span>. Admins are not limited.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Recent contacts – inline popup on View */}
          <div className="bg-surface border border-border rounded-lg shadow-sm overflow-hidden flex flex-col">
            <div className="px-4 py-3 border-b border-border bg-gray-50/40">
              <h2 className="text-sm font-semibold text-foreground">Recent Contacts</h2>
            </div>
            <div className="flex-1">
              <ul className="divide-y divide-border">
                {data.recentContacts.length === 0 ? (
                  <li className="px-4 py-8 text-sm text-muted-foreground text-center">No recent contacts.</li>
                ) : (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground truncate">
                          {c.title || "No title"} • {c.department || "No department"}
                        </p>
                      </div>
                      <button
          ...
          <div className="bg-surface border border-border rounded-lg shadow-sm overflow-hidden flex flex-col">
            ...
            <div className="flex-1">
              <ul className="divide-y divide-border">
                {data.recentAgencies.length === 0 ? (
                  <li className="px-4 py-8 text-sm text-muted-foreground text-center">No agencies found.</li>
                ) : (
      ...
      {selectedContact && (
        <div className="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-background rounded-xl shadow-xl border border-border overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm text-foreground">{selectedContact.title || "No title"}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Department</p>
                  <p className="text-sm text-foreground">{selectedContact.department || "No department"}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-foreground">{selectedContact.phone || "Not available"}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm text-foreground">{selectedAgency.type || "—"}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Location</p>
                  <p className="text-sm text-foreground">{selectedAgency.county ? `${selectedAgency.county}, ${selectedAgency.state_code || selectedAgency.state}` : selectedAgency.state}</p>
                </div>
              </div>
              ...
              <div>
                ...
                {selectedAgency.website ? (
                  <a href={selectedAgency.website} target="_blank" rel="noreferrer" className="text-sm text-primary hover:underline break-all">
                    {selectedAgency.website}
                  </a>
                ) : (
              ...
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Phone</p>
                <p className="text-sm text-foreground">{selectedAgency.phone || "Not available"}</p>
              </div>

              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Address</p>
                <p className="text-sm text-foreground">{selectedAgency.physical_address || "Not available"}</p>
              </div>

              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Population</p>
                <p className="text-sm text-foreground">{selectedAgency.population ? selectedAgency.population.toLocaleString() : "—"}</p>
              </div>
            </div>
          </div>
        </div>
